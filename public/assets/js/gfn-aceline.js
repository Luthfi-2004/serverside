(function () {
    ...
    function dp() {
        ...
        var $f = $("#fDate");
        if ($f.length) {
            $f.datepicker({ format: "yyyy-mm-dd", autoclose: true, orientation: "bottom" });
        }
        var $g = $("#gfnDate");
        if ($g.length) {
            $g.datepicker({
    ...
    async function checkDuplicate(date, shift) {
        if (!(window.aceRoutes && aceRoutes.gfnExists)) return false; // fallback: jangan blokir
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if ($) {
            $(document).on("submit", "#form-greensand", async function (e) {
                ...
                if (!d) return; // biar server yang validasi
                ...
                this.submit(); // aman -> submit beneran
            });
        }
    });
