$(function () {
    ...
    const errorHandler = {
        ...
        apply(errs) {
            const map = {
                mm: { type: "group", target: "#mm_error", groupBtns: ["#mm1_btn", "#mm2_btn"] },
                mix_ke: { type: "input", target: "#mix_ke_error", control: "#mix_ke" },
                mix_start: { type: "input", target: "#mix_start_error", control: "#mix_start" },
                mix_finish: { type: "input", target: "#mix_finish_error", control: "#mix_finish" },
                rs_time: { type: "input", target: "#rs_time_error", control: "#rs_time" }
            };
            let general = [];
    ...
    const formManager = {
        ...
        fill(data) {
            ...
            const fields = [
                "mm_p", "mm_c", "mm_gt", "mm_cb_mm", "mm_cb_lab", "mm_m", "mm_bakunetsu", "mm_ac", "mm_tc", "mm_vsd", "mm_ig",
                "mm_cb_weight", "mm_tp50_weight", "mm_ssi", "add_m3", "add_vsd", "add_sc", "bc12_cb", "bc12_m", "bc11_ac",
                "bc11_vsd", "bc16_cb", "bc16_m", "rs_type", "bc9_moist", "bc10_moist", "bc11_moist", "bc9_temp", "bc10_temp", "bc11_temp"
            ];
    ...
    // CRUD operations
    $(document).off("click", ".btn-add-gs").on("click", ".btn-add-gs", function () {
        formManager.reset();
        $("#modal-greensand").modal("show");
    });

    $(document).off("click", ".btn-edit-gs").on("click", ".btn-edit-gs", function () {
        errorHandler.clear();
    ...
    $("#gsForm").off("submit").on("submit", function (e) {
        e.preventDefault();
        errorHandler.clear();
    ...
    // Delete operations
    $(document).off("click", ".btn-delete-gs").on("click", ".btn-delete-gs", function () {
    ...
    $("#confirmDeleteYes").off("click").on("click", function () {
        ...
        $.post(`${greensandRoutes.base}/${pendingDeleteId}`, { _method: "DELETE" })
            ...
            .always(() => { pendingDeleteId = null; });
    });

    // Filter and search setup
    $("#filterDate").datepicker({ format: "dd-mm-yyyy", autoclose: true, orientation: "bottom" });
    $("#btnSearch, #btnQuickSearch").off("click").on("click", reloadAll);
    $("#keywordInput").off("keydown").on("keydown", e => {
    ...
    const baseColumns = [
        ...
        { data: "date", name: "date" }, { data: "shift", name: "shift" }, { data: "mm", name: "mm" },
        { data: "mix_ke", name: "mix_ke" }, { data: "mix_start", name: "mix_start" }, { data: "mix_finish", name: "mix_finish" },
        { data: "mm_p", name: "mm_p" }, { data: "mm_c", name: "mm_c" }, { data: "mm_gt", name: "mm_gt" },
        { data: "mm_cb_mm", name: "mm_cb_mm" }, { data: "mm_cb_lab", name: "mm_cb_lab" }, { data: "mm_m", name: "mm_m" },
        { data: "mm_bakunetsu", name: "mm_bakunetsu" }, { data: "mm_ac", name: "mm_ac" }, { data: "mm_tc", name: "mm_tc" },
        { data: "mm_vsd", name: "mm_vsd" }, { data: "mm_ig", name: "mm_ig" }, { data: "mm_cb_weight", name: "mm_cb_weight" },
        { data: "mm_tp50_weight", name: "mm_tp50_weight" }, { data: "mm_ssi", name: "mm_ssi" }, { data: "add_m3", name: "add_m3" },
        { data: "add_vsd", name: "add_vsd" }, { data: "add_sc", name: "add_sc" }, { data: "bc12_cb", name: "bc12_cb" },
        { data: "bc12_m", name: "bc12_m" }, { data: "bc11_ac", name: "bc11_ac" }, { data: "bc11_vsd", name: "bc11_vsd" },
        { data: "bc16_cb", name: "bc16_cb" }, { data: "bc16_m", name: "bc16_m" }, { data: "rs_time", name: "rs_time" },
        { data: "rs_type", name: "rs_type" }, { data: "bc9_moist", name: "bc9_moist" }, { data: "bc10_moist", name: "bc10_moist" },
        { data: "bc11_moist", name: "bc11_moist" }, { data: "bc9_temp", name: "bc9_temp" }, { data: "bc10_temp", name: "bc10_temp" },
    ...
    const summaryManager = {
        ...
        render(summary) {
            ...
            const colIndex = {
                mm_p: 7, mm_c: 8, mm_gt: 9, mm_cb_mm: 10, mm_cb_lab: 11, mm_m: 12, mm_bakunetsu: 13, mm_ac: 14,
                mm_tc: 15, mm_vsd: 16, mm_ig: 17, mm_cb_weight: 18, mm_tp50_weight: 19, mm_ssi: 20, add_m3: 21,
                add_vsd: 22, add_sc: 23, bc12_cb: 24, bc12_m: 25, bc11_ac: 26, bc11_vsd: 27, bc16_cb: 28,
                bc16_m: 29, bc9_moist: 32, bc10_moist: 33, bc11_moist: 34, bc9_temp: 35, bc10_temp: 36, bc11_temp: 37
            };
    ...
    function makeDt($el, url) {
        ...
        return $el.DataTable({
            processing: true, serverSide: true, deferRender: true, scrollX: true, autoWidth: false,
            searching: true, orderMulti: false, searchDelay: 350, pagingType: "simple_numbers",
            pageLength: 25,
            ...
            order: isAll ? [[3, "asc"], [1, "desc"]] : [[1, "desc"]],
            columns: baseColumns,
            stateSave: false,
                ...
                if (isAll) {
                    ...
                    $tbody.find("tr").each(function () {
                        ...
                        if (prevMM !== null && prevMM !== mmText) {
                            for (let i = 0; i < 3; i++) {
                                $tr.before(`<tr class="mm-spacer"><td colspan="${cells.length}" style="height:10px;border:none;padding:0;"></td></tr>`);
                            }
                        }
                        prevMM = mmText;
                    });
    ...
    function reloadAll() {
        ...
        if (helpers.getActiveTab() === "all") setTimeout(() => summaryManager.load(), 500);
    }
