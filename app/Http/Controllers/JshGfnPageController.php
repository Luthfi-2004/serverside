...
class JshGfnPageController extends Controller
{
    ...
    public function index(Request $req)
    {
        ...
        // Ambil rekap terbaru sesuai filter (kalau ada)
        $latestRecap = TotalGfn::query()
            ->when($start, fn($qq) => $qq->whereDate('gfn_date', '>=', $start))
            ->when($end,   fn($qq) => $qq->whereDate('gfn_date', '<=', $end))
        ...
        if (!$latestRecap) {
            return view('jsh-gfn.index', [
                ...
                'displayRows'   => collect(),
                'displayRecap'  => null,
                'filters'       => compact('start','end','shift','q'),
            ]);
        }

        // Tampilkan detail utk kombinasi (gfn_date, shift) dari rekap yang terpilih
    ...
    private function setDisplayForLatest($gfnDate, $shift): array
    {
        ...
        $displayRecap = $recap ? [
            ...
            'total_pi'              => (float) ($recap->total_percentage_index ?? $ordered->sum('percentage_index')),
        ] : null;
